...
function dockAlign {
	...
	if alignPos:Z >= dock_start {
		...
		set vWantZ to -max(dock_dockV, dock_apchV * vScaleZ).//jb comment out
		//print "This is close enough - docking ponez LMP.". //jb add
		//print "Mission complete. Let's go to next objective...".//jb add
		//return.//jb add
	} else {
...
function dockApproach {
	...
	if not dockComplete(dockPort) {
		...
		//print "dockApproach: aprchPos:Z = " + aprchPos:Z + ", dock_final = " + dock_final.//jb add.
		//if aprchPos:Z < 20 { //jb add
			//print "aprchPos:Z = " + aprchPos:Z + ", dock_final = " + dock_final.//jb add.
		//	print "dockApproach: aprchPos:Z < 20 so waiting for 10 seconds".//jb add.
		//	wait 10.
		//}
		...
		if aprchPos:Z < dock_final {
			if not dockPending(dockPort) {
				...
				//set vWantZ to -dock_dockV. //jb commented out
				print "This is close enough - docking ponez LMP.". //jb add
				print "Mission complete. Let's go to next objective...".//jb add
				return.//jb add
			} else {
