...
until DockingDone {
	if hastarget and ship:status = "ORBITING" and TargetVessel:distance < kuniverse:defaultloaddistance:orbit:unpack {
		...
		if dock_myPort <> 0 {
			...
			uiBanner("xpursue", "Pursueing " + dock_station:name).
			...
			uiBanner("xpursue", "Pursuit complete").
			dockFinish().
		} else {
			uiError("xpursue", "No suitable pursuit port; try moving closer?").
		}
		DockingDone on.
		...
		uiWarning("xpursue", "Target too far, approaching.").
		run approach.
	} else if hastarget and TargetVessel:distance >= MaxDistanceToApproach {
		uiError("xpursue", "Target too far, `run rendezvous` instead.").
		DockingDone on.
	} else {
		uiError("xpursue", "No target selected").
		DockingDone on.
	}
}
